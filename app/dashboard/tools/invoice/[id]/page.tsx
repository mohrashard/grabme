...
export default function InvoicePrintPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchInvoice = async () => {
            const res = await getInvoiceAction(id)
    ...
    if (!invoice) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center min-h-screen bg-slate-100">
                <h1 className="text-2xl font-black text-slate-800">Invoice Not Found</h1>
            ...
            <div className="w-full overflow-x-auto print:overflow-visible">
                ...
                <header className="flex justify-between items-start mb-12">
                    ...
                    <div className="text-right">
                        <h2 className="text-4xl font-black text-indigo-950 uppercase tracking-tighter">INVOICE</h2>
                        <p className="text-sm font-bold text-slate-400 mt-1">#{invoice.id.split('-')[0].toUpperCase()}</p>
                        <div className="mt-4 flex flex-col items-end gap-1">
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Issue Date</p>
                            <p className="text-sm font-bold text-slate-900">{new Date(invoice.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-2 gap-8 mb-12">
                    <div className="bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                        ...
                        <h3 className="text-lg font-black text-slate-900">{invoice.customer_name}</h3>
                        {invoice.customer_contact && (
                            <p className="text-sm font-bold text-slate-600 mt-1">{invoice.customer_contact}</p>
                        )}
                    </div>
                ...
                <div className="mt-auto pt-8 border-t-2 border-slate-100 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs font-bold text-slate-500">Please pay within 7 days of invoice issue date.</p>
                    </div>
