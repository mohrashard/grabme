...
export default function estimateBuilderPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const toastId = toast.loading('Generating estimate...')
        ...
        const estimateData = {
            ...
            job_description: validItems[0]?.description || 'estimate', // Satisfies the not-null constraint
            line_items: validItems
        }

        const res = await createestimateAction(estimateData)
        if (res.success && res.data) {
            toast.success('Estimate generated successfully!', { id: toastId })
            router.push(`/dashboard/tools/estimate/${res.data.id}`)
        } else {
            toast.error(res.error || 'Failed to create estimate.', { id: toastId })
            ...
            <div className="p-6 lg:p-12 max-w-4xl mx-auto space-y-8">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                        <FileText className="w-6 h-6 text-indigo-600" /> New Estimate
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">Create a professional branded estimate for your client.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="p-8 bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        >
                            {isSubmitting ? 'Generating...' : <><Send className="w-4 h-4" /> Generate Estimate</>}
                        </button>
                    </div>
                </form>
            </div>
